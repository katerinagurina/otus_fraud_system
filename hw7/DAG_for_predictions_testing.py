...
with DAG(
    ...
    ab_testing = DataprocCreatePysparkJobOperator(
        ...
        main_python_file_uri=f's3a://bucket-mlops-fraud-system/scripts/AB_test_for_predictions_testing.py',
        connection_id = ycSA_connection.conn_id,
        dag=ingest_dag,
        properties = {'spark.submit.deployMode': 'cluster',
                    'spark.yarn.dist.archives': f's3a://{YC_SOURCE_BUCKET}/pyspark_hw7.tar.gz#venv1',
                    'spark.yarn.appMasterEnv.PYSPARK_PYTHON': './venv1/bin/python',
                    'spark.yarn.appMasterEnv.PYSPARK_DRIVER_PYTHON': './venv1/bin/python'}
    )  
    ...
    create_spark_cluster >>ab_testing>>delete_spark_cluster
